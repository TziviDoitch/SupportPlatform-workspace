...
ומסלול happy-path אחד מקצה-לקצה (`dotnet test` — ראו [`ARCHITECTURE.md`](ARCHITECTURE.md) §8.5).
...
| 1.6 | מצבי טעינה/ריק/שגיאה | (טעינה) הריצו חיפוש; (ריק) ראו 2.2; (שגיאה) עצרו את ה-api והריצו | טעינה: spinner. שגיאה: toast יחיד עם `title`+`detail`+`traceId`. |
...
| 3.2 | הרצה מחדש | ברשימה, "הרץ" על שורה | מוצגים כותרת סיכום + `ResultsPanel` מלא (גרף + טבלה); `lastRunAt`/`lastRunRowCount` מתעדכנים. |
